...
- The Add Employee modal markup does not match the JavaScript handler.
- The script expects these IDs: `addEmpFirstName`, `addEmpLastName`, `addEmpEmail`, `addEmpDept`, `addEmpRole`, `addEmpDate`, `saveEmployeeBtn`, and `addEmpFeedback`.
- The current HTML inputs in the modal do not have those IDs.
- The Save button also does not have `id="saveEmployeeBtn"`.
- Instead, the button calls `document.getElementById('employeeForm').submit()` inline, which bypasses the JS `submitEmployeeForm` fetch flow entirely.
- Result: employee creation from the UI is effectively non-functional.
...
1. Fix the Add Employee modal markup and bind submission to the form, not inline `form.submit()`.
